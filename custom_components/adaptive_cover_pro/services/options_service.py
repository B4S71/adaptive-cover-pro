...
FIELD_VALIDATORS: dict[str, Any] = {
    ...
    # Geometry + runtime — louvered roof / pergola. Ranges come straight from the
    # _RANGE_LR_* constants (the same source the policy's geometry schema uses),
    # so set_option/runtime validation and the UI sliders stay in lockstep.
    # CONF_LR_SHADE_AIRFLOW backs the "Shade Airflow" option-backed switch.
    CONF_LR_AXIS_AZIMUTH: _num(*_RANGE_LR_AXIS_AZIMUTH),
    CONF_LR_PLANE_PITCH: _num(*_RANGE_LR_PLANE_PITCH),
    CONF_LR_ROOF_HEIGHT: _num(*_RANGE_LR_ROOF_HEIGHT),
    CONF_LR_PROTECTED_HEIGHT: _num(*_RANGE_LR_PROTECTED_HEIGHT),
    CONF_LR_FOOTPRINT_X: _num(*_RANGE_LR_FOOTPRINT),
    CONF_LR_FOOTPRINT_Y: _num(*_RANGE_LR_FOOTPRINT),
    CONF_LR_SLAT_CHORD: _num(*_RANGE_LR_SLAT_CM),
    CONF_LR_SLAT_THICKNESS: _num(*_RANGE_LR_SLAT_THICKNESS),
    CONF_LR_SLAT_SPACING: _num(*_RANGE_LR_SLAT_CM),
    CONF_LR_THETA_MIN: _num(*_RANGE_LR_THETA),
    CONF_LR_THETA_MAX: _num(*_RANGE_LR_THETA),
    CONF_LR_SHADE_AIRFLOW: _bool_v(),
    CONF_LR_PARK_AT_DEFAULT: _bool_v(),
    ...
    **{
    ...
    # Morning-position keys are louvered-roof-scoped (rendered in the policy
    # geometry schema, not the shared position/behaviour steps), so their ranges
    # come from the const _RANGE_MORNING_* constants directly, mirroring the
    # CONF_LR_* keys, rather than from OPTION_RANGES.
    CONF_MORNING_POSITION: _num(*_RANGE_MORNING_POSITION),
    ...
    CONF_MORNING_POSITION_LEAD: _num(*_RANGE_MORNING_LEAD),
    CONF_MORNING_POSITION_HOLD: _num(*_RANGE_MORNING_HOLD),
